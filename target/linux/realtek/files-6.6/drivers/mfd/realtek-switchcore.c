...
/* Common */
static void rtl_read_chip_name(struct regmap *map, unsigned int reg,
...
static void rtl_read_chip_info(struct regmap *map, unsigned int reg,
...
static void rtl838x_probe_model_name(const struct realtek_switchcore_ctrl *ctrl)
{
	...
	rtl_read_chip_name(ctrl->map, RTL838X_REG_MODEL_NAME_INFO, &model_id, &model_suffix);
	...
	rtl_read_chip_info(ctrl->map, RTL838X_REG_CHIP_INFO, &chip_rev, &rl_id);
...
static void rtl839x_probe_model_name(const struct realtek_switchcore_ctrl *ctrl)
{
	...
	rtl_read_chip_name(ctrl->map, RTL839X_REG_MODEL_NAME_INFO, &model_id, &model_suffix);
	rtl_read_chip_info(ctrl->map, RTL839X_REG_CHIP_INFO, &chip_rev, &rl_id);
	rtl_swcore_chip_print(ctrl->dev, model_id, model_suffix, chip_rev, rl_id);
}
